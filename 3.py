...
new_string = ''
for idx, char in enumerate(mess[3:-3]):
    if char in small_letters:
        index = idx + 3
        bodyguards = [
            mess[index - 3], mess[index - 2], mess[index - 1],
            mess[index + 1], mess[index + 2], mess[index + 3],
        ]
        next_to_bodyguards = []
        try:
            next_to_bodyguards.append(mess[index - 4])
        except IndexError:
            pass
        try:
            next_to_bodyguards.append(mess[index + 4])
        ...
        if all([bodyguard in big_letters for bodyguard in bodyguards]):
            if all([letter in small_letters for letter in next_to_bodyguards]):
                new_string += char
